...
<motion.div
  className="relative z-10 max-w-4xl text-center h-[100vh] flex flex-col items-center justify-center px-6 md:px-0"
...
>
  <div>
    {/* Título */}
    <motion.h2
      className="text-neutral-800 text-4xl md:text-6xl lg:text-7xl font-extrabold mb-8 tracking-wider"
      initial={{ opacity: 0, y: -20 }}
      whileInView={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.2, duration: 0.6 }}
    >
     AGENTES<span className="text-gray-500"> GLYNNE</span>
    </motion.h2>
    ...
    >
      Con el panel de GLYNNE, diseña tu propio agente de inteligencia artificial, ajusta su personalidad, prueba su rendimiento y exporta su comportamiento directamente a tus aplicaciones. 
      Todo esto potenciado por el motor de procesamiento generativo de GLYNNE, que permite que tu agente entienda contextos, responda dinámicamente y se integre de forma inmediata. 
      Tu IA, totalmente personalizable, lista para impactar y automatizar cualquier flujo de trabajo empresarial.
    </motion.p>
  </div>
</motion.div>

<motion.h2
  className="text-neutral-800 text-l md:text-xl lg:text-xl font-extrabold mb-2 tracking-wider"
  initial={{ opacity: 0, y: -20 }}
  whileInView={{ opacity: 1, y: 0 }}
  transition={{ delay: 0.2, duration: 0.6 }}
>
  PANEL <span className="text-gray-500">DESARROLLO</span>
</motion.h2>

<motion.p
  className="text-gray-600 text-sm md:text-base lg:text-base mb-6"
  initial={{ opacity: 0, y: -10 }}
  whileInView={{ opacity: 1, y: 0 }}
  transition={{ delay: 0.3, duration: 0.6 }}
>
  En esta primera sección generarás el acceso al LLM y podrás personalizar y ajustar todas las características del agente, incluyendo su nombre, personalidad, rol y especialidad.
</motion.p>
...
<p className="text-gray-500 text-xs mt-2">
  ...
  <button
    onClick={async () => {
      try {
        // 1) Guardar configuración del agente en Supabase
        await saveUserAgentConfig(form);
        addLog("✅ Configuración guardada en Supabase", "success");
      } catch (err) {
        addLog("⚠️ No se pudo guardar configuración: " + err.message, "error");
    ...
    className="bg-black text-white px-5 py-2 text-sm rounded-xl shadow-md hover:shadow-lg transition-all duration-300"
  >
    ⚡ Execute Agent
  </button>
</div>
