    ...
    <>
      ...
      <AnimatePresence>
        {openModal && !loadingScreen && (
          ...
          >
            <motion.div
              className="relative bg-white rounded-3xl shadow-2xl w-full max-w-4xl px-8 py-8 text-gray-800 overflow-y-auto max-h-[90vh]"
            ...
            >
              ...
              <button
                ...
                className="absolute top-4 right-4 text-gray-500 hover:text-black text-xl font-bold transition-colors"
              >
                ✕
              </button>
              ...
              <div className="flex flex-col items-center gap-4">
                <motion.h2
                  ...
                  className="font-bold text-center text-black text-sm"
                >
                ...
                <form onSubmit={handleSubmit} className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                  {[
                    ...
                    <div key={field.name} className="relative">
                      <input
                        ...
                        className="w-full p-3 rounded-xl text-sm bg-white border border-gray-300 focus:outline-none z-10"
                        required
                      />
                      {focused === field.name && (
                        <span className="absolute inset-0 rounded-xl pointer-events-none bg-gradient-to-r from-green-400 via-blue-500 via-yellow-400 to-pink-500 animate-[shine_2.5s_linear_infinite] opacity-40" />
                      )}
                    </div>
                  ))}

                  <select name="pais" value={formData.pais} onChange={handleChange} className="p-3 rounded-xl border text-sm" required>
                    <option value="">Selecciona tu país</option>
                    {paises.map((p) => <option key={p}>{p}</option>)}
                  </select>

                  <select name="ciudad" value={formData.ciudad} onChange={handleChange} className="p-3 rounded-xl border text-sm" required>
                  ...
                  <motion.button
                    ...
                    type="submit"
                    className="col-span-full mt-3 px-8 py-3 rounded-full font-semibold text-white bg-black shadow-xl overflow-hidden text-sm relative"
                  >
      ...
      <AnimatePresence>
        {loadingScreen && (
          <motion.div className="fixed inset-0 bg-white flex items-center justify-center z-[9999]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 2 }}>
