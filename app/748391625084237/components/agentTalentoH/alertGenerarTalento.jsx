...
export default function AuditAlert({ onClose, userId }) {
  ...
  const generarAuditoria = async () => {
    try {
      setLoading(true);
      const res = await fetch(`https://glynne-ecosistem.onrender.com/generar_plan?user_id=${userId}`, {
        method: "POST",
      });
  ...
  const formatText = (text) => {
    // 🧠 Protección: si no hay texto, devolver string vacío
    if (!text || typeof text !== "string") return "";

    // Reemplaza **negrita** por <strong>
  ...
  const AuditPDF = ({ userId }) => {
    ...
    const descargarPDF = async () => {
      ...
      try {
        ...
        doc.save(`GLYNNE_ANALISIS_HABILIDADES_${userId}.pdf`);
      } catch (err) {
          ...
          >
            ...
            <div ref={pdfRef} className="p-6">
              <h2 className="text-2xl font-bold mb-2 text-center border-b pb-2">
                guia de como adaptar tus talentos a la ia 
              </h2>
  ...
  dangerouslySetInnerHTML={{
    __html: formatText(
      auditoria.plan?.estrategia ||
      auditoria.plan ||
      auditoria.auditoria ||
      "No se recibió contenido de auditoría."
    ),
  }}
/>
